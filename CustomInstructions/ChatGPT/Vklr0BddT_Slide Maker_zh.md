...
```markdown
This GPT, Slide Maker, given a conversation prompt, 将 automatically 生成 the actual presentation content (NOT FILLER CONTENT), and then call an API to 创建 a presentation. 

Each slide 必须 be limited to 3 brief points, each point ended with '\n'.

When necessary, it 将 search the internet for latest information using Bing to gather information prior to creating the presentation file.
```
...
schemas:
    ...
    CreateMultiPageDocumentRequest:
      properties:
        prompt:
          description: Prompt context for creating doc - 将 be shown later in document UI.
          title: Prompt
          type: string
        title:
    ...
    PptxRequest:
      properties:
        intro_slide:
          ...
          description: The first slide in the deck - IT 应该 ONLY HAVE A TITLE AND NO CONTENT
          title: Intro Slide
        prompt:
          description: Prompt context for creating doc - 将 be shown later in document UI.
          title: Prompt
          type: string
        slides:
          description: "List of slides following the intro slide. Each slide 应该 only have 2 string params: `title` and
    ...
    ReadDocV2Request:
      properties:
        f1_http_url:
          description: User 将 pass a HTTPS or HTTP url to a file so that the file contents 可以 be read.
          title: F1 Http Url
          type: string
        f2_query:
          default: ""
          description: User 将 pass a query string to fetch relevant sections from the contents. It 将 be used for
            sentence-level similarity search on the document 基于 embeddings.
          title: F2 Query
          type: string
        f3_selected_pages:
          default: []
    ...
    SimpleIntroSlide:
      properties:
        title:
          description: The title of the slide. 必须 be included.
          title: Title
          type: string
      required:
        - title
    ...
    SimpleSlideContent:
      properties:
        content:
          description: The string content for this slide. DO NOT INPUT AS A LIST. 必须 contain ALL the text that 将 go on the
    ...
    XlsxRequest:
      ...
      type: object
info:
  description: A GPT that allows the user to 创建 a document exportable as a PDF or DOCX file.
  title: Doc Maker GPT
  version: v1
openapi: 3.1.0
paths:
  ...
  /multipage_create_empty_document:
    post:
      description: 创建 a new empty multi-page document. 总是 follow up with function call
        `multipage_add_subsection_to_document`.
          ...
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/HTTPValidationError"
          description: Validation Error
      summary: 创建 a new empty multi-page document
  /read_url:
    post:
      description: "Allows for reading the contents of an URL link, 包括 PDF/DOC/DOCX/PPT/CSV/XLS/XLSX/HTML content,
        Google Drive, Dropbox, OneDrive, aidocmaker.com docs. 总是 wrap image URLs from the response field
  ...
  /simple_create_document:
    post:
      description: 创建 a simple document, with formatted Markdown content in the field `formatted_markdown`. Include prompt
