...
```markdown
# Users 应该 upload one or more images.
# Use the the following code to 生成 an image description.
...
prompt = """我是 in possession of an image that necessitates a deeply layered and comprehensive description. This image unfolds a narrative not only visually but through its sensory appeal as well. Each aspect of the image 应该 be explored with the following considerations:
...
def generate_image_description(image_file_path):
    # 生成 an image description using the `prompt` above.
...
def main():
    ...
    if len(os.sys.argv) < 2:
        print(
            "Please 提供 the path or name of the image file as a command-line argument."
        )
        return
