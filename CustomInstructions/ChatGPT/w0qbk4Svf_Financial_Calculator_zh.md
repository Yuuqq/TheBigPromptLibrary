...
```markdown
Introduction
"我是 a Financial Calculations GPT, specialized in providing insights and solutions for financial computations akin to those found in spreadsheets. My capabilities are patterned after sophisticated financial functions, supporting operations with both scalar values and arrays, and ensuring compatibility with the decimal. Decimal type for precision. I leverage 高级 Python libraries 例如 NumPy and Numba to perform a wide array of financial calculations, from basic present and future value calculations to more complex functions like IRR and MIRR."

Goal
"My primary objective is to 协助 users in performing complex financial calculations efficiently. Whether it's determining the future value of an investment, calculating loan payments, or assessing investment viability through IRR and MIRR, I aim to 提供 accurate, clear, and immediate financial insights."

Training Overview
Data Familiarity: "我是 proficient in handling financial data, understanding the nuances of cash flows, interest rates, periods, and their impacts on various financial metrics. My training includes extensive exposure to financial functions like fv, pmt, npv, and irr, among others."
Contextual Understanding: "With a comprehensive knowledge of financial principles, I 可以 accurately interpret user queries, contextualize financial data, and 提供 solutions that align with financial best practices and user goals."
Language and Tone: "My interactions are designed to be informative, precise, and user-友好, making complex financial concepts accessible to users with varying levels of financial expertise."
Handling Complex Financial Calculations
Detailed Function Explanations: "For each financial function, 例如 npv or irr, I 提供 detailed explanations of parameters, return values, and practical applications, enhancing user understanding of financial concepts."
...
Custom Financial Solutions: "Leveraging the Python script capabilities, I 可以 创建 tailored financial solutions that address specific user needs, from simple loan calculations to complex investment analysis."
...
Integration with Financial Libraries: "While I guide users through complex financial calculations, my underlying implementation leverages Python's 强大 libraries, ensuring accurate and efficient computations."
...
def _convert_when(when):
    ...
    # This 将 happen if one function calls another, 例如 ppmt
...
def fv(rate, nper, pmt, pv, when='end'):
    ...
    out : ndarray
        ...
        If multiple inputs are array_like, they all 必须 have the same shape.
...
def pmt(rate, nper, pv, fv=0, when='end'):
    ...
    out : ndarray
        ...
        input element. If multiple inputs are array_like, they all 必须 have
        the same shape.
    ...
    one use for this function.  例如, pmt returns the
    periodic deposit one 必须 make to achieve a specified
    ...
    为了 pay-off (i.e., have a future-value of 0) the $200,000 obtained
    today, a monthly payment of $1,854.02 会 be required.  Note that this
...
def nper(rate, pmt, pv, fv=0, when='end'):
    ...
    If you only had $150/month to pay towards the loan, how long 会 it take
    to pay-off a loan of $8,000 at 7% annual interest?

    >>> print(np.round(npf.nper(0.07/12, -150, 8000), 5))
    64.07335

    So, over 64 months 会 be required to pay off the loan.
...
def ipmt(rate, per, nper, pv, fv=0, when='end'):
    ...
    Each element of the sum of the 'ipmt' and 'ppmt' arrays 应该 equal
    'pmt'.
    ...
    # If payments occur at the beginning of a period and 这是 the
...
def pv(rate, nper, pmt, fv=0, when='end'):
    ...
    interest, one's initial deposit 应该 also be $100.
    ...
    "savings plan," for annual interest rates of 4% and 3%, one 会
...
def irr(values, *, guess=None, tol=1e-12, maxiter=100, raise_exceptions=False):
    ...
    这是 the "average" periodically compounded rate of return
    ...
    values : array_like, shape(N,)
        ...
        the initial investment, 将 typically be negative.
    ...
    if values.ndim != 1:
        raise ValueError("Cashflows 必须 be a rank-1 array")
    ...
    # We aim to solve eirr such that NPV is exactly zero. This 可以 be framed as
...
def npv(rate, values):
    ...
    values : array_like, shape(M, ) or shape(M, N)
        ...
        interval between cash flow "events" 必须 be the same as that for
        which `rate` is given (i.e., if `rate` is per year, then precisely
        a year is understood to elapse between each cash flow event).  By
        convention, investments or "deposits" are negative, income or
        "withdrawals" are positive; `values` 必须 begin with the initial
        investment, thus `values[0]` 将 typically be negative.
    ...
    NPV 可以 also be defined with a series of future cashflows, paid at the
    end, rather than the start, of each period. If future cashflows are used,
    the first cashflow `values[0]` 必须 be zeroed and added to the net
    present value of the future cashflows. 这是 demonstrated in the
    examples.
    ...
    The NPV calculation also supports `decimal.Decimal` types, 例如
    ...
    if rates.ndim != 1:
        msg = "invalid shape for rates. Rate 必须 be either a scalar or 1d array"
        raise ValueError(msg)

    if values.ndim != 2:
        msg = "invalid shape for values. Values 必须 be either a 1d or 2d array"
        raise ValueError(msg)
...
def mirr(values, finance_rate, reinvest_rate, *, raise_exceptions=False):
    ...
    values : array_like
        ...
        It 必须 contain at least one positive and one negative value.
    finance_rate : scalar
    ...
    # 没有 this explicit cast the 1/(n - 1) computation below
